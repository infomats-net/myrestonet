      ...
      {cartCount > 0 && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 w-full max-w-xl px-6 z-50">
          <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
            ...
            <SheetContent side="bottom" className="h-[70vh] rounded-t-[4rem] px-6 md:px-10 pt-12 overflow-hidden flex flex-col">
              <SheetHeader className="mb-6 shrink-0 text-center md:text-left">
                <SheetTitle className="text-3xl md:text-4xl font-black flex items-center gap-3 justify-center md:justify-start" style={{ fontFamily: designStyles['--heading-font'] as string }}>
                  <ShoppingBag className="h-8 w-8 text-primary" /> Your Order
                </SheetTitle>
              </SheetHeader>
              ...
              <SheetFooter className="shrink-0 pt-6 border-t bg-white mt-auto">
                <div className="w-full space-y-6">
                  <div className="flex justify-between font-black text-3xl md:text-4xl text-slate-900"><span>Total</span><span style={{ color: designStyles['--primary'] as string }}>{currencySymbol}{subtotal.toFixed(2)}</span></div>
                  <Button className="w-full h-16 md:h-20 text-xl md:text-2xl font-black rounded-[2rem] md:rounded-[2.5rem] shadow-xl transition-all hover:scale-[1.02] active:scale-[0.98] group" style={{ backgroundColor: designStyles['--primary'] as string }}>
                    Checkout Currently Disabled <ArrowRight className="ml-2 h-6 w-6 group-hover:translate-x-1 transition-transform" />
                  </Button>
                </div>
              </SheetFooter>
            </SheetContent>
