...
import { ChevronRight, UtensilsCrossed, Sparkles, LayoutGrid } from 'lucide-react';
 ...
 * Original high-end curated catalog with large imagery.
 */
export function MenuStyle1({ menus, allMenuItems, currencySymbol, theme, addToCart }: MenuLayoutProps) {
  return (
...
/**
 * Style 2: Grid Cards
 * Compact multi-column grid, great for visual browsing.
 */
export function MenuStyle2({ menus, allMenuItems, currencySymbol, theme, addToCart }: MenuLayoutProps) {
  return (
              ...
              <Card key={item.id} className="border-none shadow-md rounded-3xl bg-white overflow-hidden hover:shadow-2xl transition-all">
                <div className="aspect-square relative">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent flex justify-between items-end">
                    ...
                    <Button size="icon" className="rounded-full h-10 w-10" style={{ backgroundColor: theme.primary }} onClick={() => addToCart(item)}><ChevronRight /></Button>
                  </div>
                </div>
 ...
 * Tabbed navigation for efficient category switching.
 */
export function MenuStyle3({ menus, allMenuItems, currencySymbol, theme, addToCart }: MenuLayoutProps) {
        ...
        {menus.map(menu => (
          <TabsContent key={menu.id} value={menu.id} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid gap-8 sm:grid-cols-2">
              {allMenuItems.filter(i => i.menuId === menu.id).map(item => (
                <div key={item.id} className="flex gap-6 p-6 bg-slate-50/50 rounded-[2rem] border border-slate-100 group hover:bg-white hover:shadow-xl transition-all">
 ...
 * Typography-led single column layout, focus on ingredients.
 */
export function MenuStyle4({ menus, allMenuItems, currencySymbol, theme, addToCart }: MenuLayoutProps) {
  return (
